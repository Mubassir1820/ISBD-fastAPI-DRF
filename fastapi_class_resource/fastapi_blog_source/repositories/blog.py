...
class BlogRepository:
    ...
    def update_blog(self, blog_id: int, blog: BlogUpdate, author_id: int) -> Optional[Blog]:
        """
        Update an existing blog by its ID.
        """
        db_blog = self.db.query(Blog).filter(Blog.id == blog_id, Blog.author_id == author_id).first()
        if not db_blog:
            raise HTTPException(status_code=404,detail="Blog not found!")
    ...
    def delete_blog(self, blog_id: int, author_id: int) -> bool:
        """
